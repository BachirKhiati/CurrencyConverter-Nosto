...
const CurrencyFormatter =
  {
    ...
    getFormatDetails: function (p) {
      const locales = CurrencyFormatter.locales
      const defaultLocales = CurrencyFormatter.defaultLocales
      const symbols = CurrencyFormatter.symbols

      let locale, currency, symbol, pattern, decimal, group, valueOnError
    ...
    getFormatter: function (p) {
      const formatDetails = CurrencyFormatter.getFormatDetails(p)
      ...
      const format = function (n, f) {
        let formattedNumber = CurrencyFormatter.toFixed(Math.abs(n), f.decimalPlaces)
        ...
        formattedNumber = f.frontPadding + segment + (typeof splitNumber[1] === 'undefined' ? '' : (f.decimal + splitNumber[1])) + f.backPadding
    ...
    formatAll: function (p) {
      const formatter = CurrencyFormatter.getFormatter(p)

      const matches = document.querySelectorAll(p.selector)
    ...
    formatEach: function (selector) {
      ...
      for (let i = 0; i < matches.length; ++i) {
        try {
          const ccy = matches[i].getAttribute('data-ccy')

          if (typeof formatters[ccy] === 'undefined') {
            formatters[ccy] = CurrencyFormatter.getFormatter({ currency: ccy })
          }

          const formatter = formatters[ccy]
    ...
    format: function (n, p) {
      const formatterFunction = CurrencyFormatter.getFormatter(p)

      return formatterFunction(n)
    },

    parse: function (str, p) {
      const decimal = CurrencyFormatter.getFormatDetails(p).decimal
